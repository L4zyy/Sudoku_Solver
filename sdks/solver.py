...
def get_grid(gray_img, mask):
    ...
    # draw_linesx = cv.HoughLinesP(sobelx, 1, np.pi/180, 250, minLineLength=300, maxLineGap=200)
    # linesx_img = np.zeros_like(sobelx)
    # for line in draw_linesx:
    #     x1, y1, x2, y2 = line.reshape(4)
    #     linesx_img = cv.line(linesx_img, (x1, y1), (x2, y2), (255, 255, 255), 2)


    # draw_linesy = cv.HoughLinesP(sobely, 1, np.pi/180, 250, minLineLength=300, maxLineGap=250)
    # linesy_img = np.zeros_like(sobely)
    # for line in draw_linesy:
    #     x1, y1, x2, y2 = line.reshape(4)
    #     linesy_img = cv.line(linesy_img, (x1, y1), (x2, y2), (255, 255, 255), 2)
...
class sdkSolver():
    ...
    def get_num_imgs(self):
        # get sudoku mask
        mask = get_mask(self.gray_img)

        # remove non-sudoku
        clear = cv.bitwise_and(self.img, self.img, mask=mask)

        # get grid image
        mix_img = get_grid(self.gray_img, mask)
    
        # find intecept points by contours
        test = np.copy(self.img)
        ...
        self.num_pads = get_num_pads(self.gray_img, mid_points)
    ...
    def get_sudoku_board_matrix(self):
        ...
        pred_mat = np.array(pred_mat).reshape(9, 9)
